...
# ??   --- not conditional operator (condition ? then : else)
    elsif (($slash eq 'm//') and /\G \? ((?:[\x81-\xFE][\x00-\xFF]|\\[?:]|[^?:])*) \? ([cgimosxp]*) /oxmsgc) {
        return e_qr('', '?','?',$1,$2);
    }
...
sub e_qr {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        $re = join '', $ope, $delimiter, "$your_gap(?:", @char, '>]}' x ($left_e - $right_e), ')(?{$Euhc::last_s_matched=0})', $end_delimiter, $modifier;
    }
    else {
        $re = join '', $ope, $delimiter, "$your_gap(?:", @char,                               ')(?{$Euhc::last_s_matched=0})', $end_delimiter, $modifier;
    }
    return $re;
}
...
sub e_qr_q {
    ...
    $modifier =~ tr/i//d;
    return join '', $ope, $delimiter, "$your_gap(?:", @char, ')(?{$Euhc::last_s_matched=0})', $end_delimiter, $modifier;
}
...
sub e_s1 {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        $re = join '', $ope, $delimiter, "($your_gap)(?:", @char, '>]}' x ($left_e - $right_e), ')(?{$Euhc::last_s_matched=1})', $end_delimiter, $modifier;
    }
    else {
        $re = join '', $ope, $delimiter, "($your_gap)(?:", @char,                               ')(?{$Euhc::last_s_matched=1})', $end_delimiter, $modifier;
    }
    return $re;
}
...
sub e_s1_q {
    ...
    $modifier =~ tr/i//d;
    return join '', $ope, $delimiter, "($your_gap)(?:", @char, ')(?{$Euhc::last_s_matched=1})', $end_delimiter, $modifier;
}
...
In present Japan, UHC is widely used on mainframe I/O, the personal computer,
