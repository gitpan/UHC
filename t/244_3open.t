...
# 引数 3 個 >:encoding(uhc), ベアワード

if ($] =~ /^5\.006/) {
    print "ok - 11 # SKIP open(FILE,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'>:encoding(uhc)','open.txt');
    print FILE "A" x 11;
    print FILE "\n";
    close(FILE);
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        if ($file eq ('A' x 11)."\r\n") {
            print "ok - 11 open(FILE,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
        else {
            print "not ok - 11 open(FILE,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
    }
    else {
        if ($file eq ('A' x 11)."\n") {
...
if ($] =~ /^5\.006/) {
    print "ok - 12 # SKIP open(FILE,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'>>:encoding(uhc)','open.txt');
    print FILE "A" x 12;
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        if ($file eq ('A' x 11)."\r\n".('A' x 12)."\r\n") {
            print "ok - 12 open(FILE,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
        else {
            print "not ok - 12 open(FILE,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
    }
    else {
        if ($file eq ('A' x 11)."\n".('A' x 12)."\n") {
...
if ($] =~ /^5\.006/) {
    print "ok - 13 # SKIP open(FILE,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'<:encoding(uhc)','open.txt');
    my $file = join('',<FILE>);
    ...
    if ($file eq ('A' x 11)."\n".('A' x 12)."\n") {
        print "ok - 13 open(FILE,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 13 open(FILE,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}

# 引数 3 個 >:crlf:encoding(uhc), ベアワード

if ($] =~ /^5\.006/) {
    print "ok - 14 # SKIP open(FILE,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'>:crlf:encoding(uhc)','open.txt');
    print FILE "A" x 14;
    print FILE "\n";
    close(FILE);
    ...
    if ($file eq ('A' x 14)."\r\n") {
        print "ok - 14 open(FILE,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 14 open(FILE,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
...
if ($] =~ /^5\.006/) {
    print "ok - 15 # SKIP open(FILE,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'>>:crlf:encoding(uhc)','open.txt');
    print FILE "A" x 15;
    ...
    if ($file eq ('A' x 14)."\r\n".('A' x 15)."\r\n") {
        print "ok - 15 open(FILE,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 15 open(FILE,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
...
if ($] =~ /^5\.006/) {
    print "ok - 16 # SKIP open(FILE,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(FILE,'<:crlf:encoding(uhc)','open.txt');
    my $file = join('',<FILE>);
    ...
    if ($file eq ('A' x 14)."\n".('A' x 15)."\n") {
        print "ok - 16 open(FILE,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 16 open(FILE,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
...
# 引数 3 個 >:encoding(uhc), 未定義の値が格納された変数

if ($] =~ /^5\.006/) {
    print "ok - 26 # SKIP open(my \$fh,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'>:encoding(uhc)','open.txt');
    print $fh "A" x 26;
    print $fh "\n";
    close($fh);
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        if ($file eq ('A' x 26)."\r\n") {
            print "ok - 26 open(my \$fh,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
        else {
            print "not ok - 26 open(my \$fh,'>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
    }
    else {
        if ($file eq ('A' x 26)."\n") {
...
if ($] =~ /^5\.006/) {
    print "ok - 27 # SKIP open(my \$fh,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'>>:encoding(uhc)','open.txt');
    print $fh "A" x 27;
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        if ($file eq ('A' x 26)."\r\n".('A' x 27)."\r\n") {
            print "ok - 27 open(my \$fh,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
        else {
            print "not ok - 27 open(my \$fh,'>>:encoding(uhc)','open.txt'); $^X $__FILE__\n";
        }
    }
    else {
        if ($file eq ('A' x 26)."\n".('A' x 27)."\n") {
...
if ($] =~ /^5\.006/) {
    print "ok - 28 # SKIP open(my \$fh,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'<:encoding(uhc)','open.txt');
    my $file = join('',<$fh>);
    ...
    if ($file eq ('A' x 26)."\n".('A' x 27)."\n") {
        print "ok - 28 open(my \$fh,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 28 open(my \$fh,'<:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}

# 引数 3 個 >:crlf:encoding(uhc), 未定義の値が格納された変数

if ($] =~ /^5\.006/) {
    print "ok - 29 # SKIP open(my \$fh,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'>:crlf:encoding(uhc)','open.txt');
    print $fh "A" x 29;
    print $fh "\n";
    close($fh);
    ...
    if ($file eq ('A' x 29)."\r\n") {
        print "ok - 29 open(my \$fh,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 29 open(my \$fh,'>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
...
if ($] =~ /^5\.006/) {
    print "ok - 30 # SKIP open(my \$fh,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'>>:crlf:encoding(uhc)','open.txt');
    print $fh "A" x 30;
    ...
    if ($file eq ('A' x 29)."\r\n".('A' x 30)."\r\n") {
        print "ok - 30 open(my \$fh,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 30 open(my \$fh,'>>:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
...
if ($] =~ /^5\.006/) {
    print "ok - 31 # SKIP open(my \$fh,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
}
else {
    open(my $fh,'<:crlf:encoding(uhc)','open.txt');
    my $file = join('',<$fh>);
    ...
    if ($file eq ('A' x 29)."\n".('A' x 30)."\n") {
        print "ok - 31 open(my \$fh,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
    else {
        print "not ok - 31 open(my \$fh,'<:crlf:encoding(uhc)','open.txt'); $^X $__FILE__\n";
    }
}
